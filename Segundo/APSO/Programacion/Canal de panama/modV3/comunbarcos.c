...
void registrarFin(int cola, int como){
	struct tipo_registro registro;
	registro.tiempo = time(0)-inicio;
	registro.cualidad = como;
	msgsnd (cola, (struct tipo_registro *) &registro,sizeof(struct tipo_registro)-sizeof(long),0);
}
