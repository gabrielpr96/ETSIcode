...
int main()
{
 ...
 //Modificacion
 /*
  	Utilizo la tubería para que canalpanama pase los pids de los gestores a cierraexclusas
 	y la fifo para que cierraexclusas devuelva cuantas veces y cuento tiempo han estado cerradas las exclusas
 */
 int pidCierraExclusas, fCierreExclusas;
 int tCerrada, nCierres;
 int tuboCierraExclusas[2];
 ...
 //Modificacion
 fCierreExclusas=open("cierre",O_RDWR);
 ...
 ///Modificacion
 pidCierraExclusas = creaprocesotub("cierraexclusas", tuboCierraExclusas, 2);
 write(tuboCierraExclusas[1], &pidgesclusae, sizeof(pidgesclusae));
 write(tuboCierraExclusas[1], &pidgesclusao, sizeof(pidgesclusao));
 ...
 system("reset");
  ...
  //Modificacion
  kill(pidCierraExclusas, 10);

  read(fCierreExclusas, &tCerrada, sizeof(tCerrada));
  read(fCierreExclusas, &nCierres, sizeof(nCierres));

  printf("Se han cerrado las eclusas %d veces\n", nCierres);
  printf("Las exclusas han estado cerradas %d segundos\n", tCerrada);
 ...
 *maxbarcos=15; //Numero de barcos que se crearan
