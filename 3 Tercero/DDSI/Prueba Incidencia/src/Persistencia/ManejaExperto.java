...
public class ManejaExperto {
    ...
    /**
     *  Devuelve una lista con todos los expertos ordenados por nombre
     * @return Lista de expertos
     * @throws SQLException
     */
    public ArrayList<Experto> listaExpertosPorNombre() throws SQLException {
        ArrayList<Experto> lista = new ArrayList<>();
        ResultSet rs = conexion.getConexionOracle().createStatement().executeQuery("SELECT * FROM EXPERTO ORDER BY NOMBRE ASC");
        while (rs.next()) {
            lista.add(new Experto(rs.getString("CODEXPERTO"), rs.getString("NOMBRE"), rs.getString("PAIS"), rs.getString("SEXO"), rs.getString("ESPECIALIDAD")));
        }
        return lista;
