...
import practica.geometria.Camino;
...
public class Algoritmos {
    ...
    public static Camino[] dijkstra(double[][] matriz, Punto[] vertices) throws Exception {
        Camino[] caminos = new Camino[vertices.length];
        int[] solucion = dijkstra(matriz, vertices.length, 0);
        for (int i = 0; i < vertices.length; i++) {
            ...
            int actual = i;
            do {
                camino.add(vertices[actual]);
                if (solucion[actual] != -1) {
                    coste += vertices[actual].distancia(vertices[solucion[actual]]);
                }
                actual = solucion[actual];
            } while (actual != -1);

            caminos[i] = new Camino(camino, coste);
        }
        return caminos;
    }

    public static int[] dijkstra(double[][] matriz, int nVertices, int origen) {
        ...
        do {
            for (int i = 0; i < nVertices; i++) {
                if (Double.isFinite(matriz[actual][i]) && (D[i] == 0 || D[i] > D[actual] + matriz[actual][i])) {
    ...
    public static void DrawDijkstra(Punto[] puntos, Lienzo lienzo, JTextArea mensaje) {
        lienzo.drawMap(puntos, puntos.length <= 20);
        mensaje.setText("Generando aristas");
        double[][] aristas = generarMatrizAdyacencia(puntos);
        mensaje.setText("Ejecutando algoritmo");
        try {
            Camino[] resultado = dijkstra(aristas, puntos);
            System.out.println(resultado);

            for (Camino camino : resultado) {
                lienzo.clearLines();
                Punto lastPunto = null;
                for (Punto newPunto : camino.getVertices()) {
                    if(lastPunto != null){
                        lienzo.addDrawLinea(new Linea(newPunto, lastPunto));
                    }
                    lastPunto = newPunto;
                }
