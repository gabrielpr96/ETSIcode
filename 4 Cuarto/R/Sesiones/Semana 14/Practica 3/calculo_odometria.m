...
%Cálculo del ángulo
theta_new=theta(i-1)+r_rueda*(dalfa1-dalfa2)/(2*l_semi);
x_new=x(i-1)+((r_rueda*(dalfa1+dalfa2)/2)*cos((theta_new+theta(i-1))/2));
y_new=y(i-1)+((r_rueda*(dalfa1+dalfa2)/2)*sin((theta_new+theta(i-1))/2));
